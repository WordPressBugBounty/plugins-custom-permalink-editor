...
class Custom_Permalink_Editor_Form
{
	...
	private function exclude_permalink_manager($post)
	{
		...
		} elseif (isset($this->permalink_metabox) && 1 === $this->permalink_metabox) {
			$check_availability = true;
	...
	{
		if (
			! isset($_REQUEST['_wp_permalink_manager_post_nonce'])
		...
		if ('post' !== $post->post_type && 'page' !== $post->post_type) {
			// Check if this post already has a custom permalink
			$existing_permalink = get_post_meta($post_id, 'cp_editor', true);
			if (empty($existing_permalink)) {
				// No existing custom URL - don't allow saving new ones (Pro feature)
				return;
			}
			// Has existing custom URL - allow it to continue working (no update allowed)
			return;
		}
	...
	private function get_permalink_html($post, $meta_box = false)
	{
		...
		if ('page' === $post->post_type) {
			...
			if (
				is_object($post_type_object) && isset($post_type_object->labels)
	...
	public function meta_edit_form($post)
	{
		// Check if this is a custom post type (not post or page)
		if ('post' !== $post->post_type && 'page' !== $post->post_type) {
			// Check if this post has an existing custom permalink
			$existing_permalink = get_post_meta($post->ID, 'cp_editor', true);
			if (empty($existing_permalink)) {
				// No existing custom URL - show pro teaser
				$this->render_pro_teaser_form($post);
				return;
			}
			// Has existing custom URL - show read-only form with pro message
			$this->render_pro_teaser_form($post, true);
			return;
		}
	...
	/**
	 * Render a teaser form for custom post types (Pro feature)
	 *
	 * @param object $post The post object.
	 * @param bool $has_existing Whether the post has an existing custom permalink.
	 */
	private function render_pro_teaser_form($post, $has_existing = false)
	{
		$cp_frontend = new Custom_Permalink_Editor_Frontend();
		$home_url = trailingslashit(home_url());

		if ($has_existing) {
			// Show the existing custom permalink (read-only)
			$existing_permalink = get_post_meta($post->ID, 'cp_editor', true);
			$post_slug = htmlspecialchars(urldecode($existing_permalink));
			$message = __('This post has an existing custom permalink. To edit or add new custom permalinks for this post type, please upgrade to the Pro version.', 'custom-permalink-editor');
		} else {
			// Show the original permalink (read-only)
			$original_permalink = $cp_frontend->original_post_link($post->ID);
			$post_slug = htmlspecialchars(urldecode($original_permalink));
			$message = __('Custom permalink for this post type is available in Pro version.', 'custom-permalink-editor');
		}

		$output = '<strong>' . __('New Permalink:', 'custom-permalink-editor') . '</strong> ';
		$output .= esc_url($home_url);
		$output .= '<span id="editable-post-name">';
		$output .= '<input type="text" value="' . esc_attr($post_slug) . '" style="width: 250px; color: #999; background: #f5f5f5;" disabled="disabled" />';
		$output .= '</span>';
		$output .= '<p style="margin-top: 10px;">';
		$output .= '<span style="color: #d63638; font-weight: 500;">' . esc_html($message) . '</span> ';
		$output .= '<a href="https://kingscrestglobal.com/contact/" target="_blank" class="button button-primary button-small">' . __('Get Pro', 'custom-permalink-editor') . '</a>';
		$output .= '</p>';
		$output .= '<style>.editor-post-permalink,.custom-permalink-editor-hidden{display:none;}</style>';
...
<?php
	...
	public function save_term($term_id)
	{
		...
		if (
			! isset($_REQUEST['_cp_editor_term_nonce'])
			&& ! isset($_REQUEST['cp_editor'])
		) {
			return;
		}
		...
		if (
			! wp_verify_nonce($_REQUEST['_cp_editor_term_nonce'], $action1)
		...
		if (isset($term) && isset($term->taxonomy)) {
			...
			if ('category' === $taxonomy_name || 'post_tag' === $taxonomy_name) {
				...
				$new_permalink = ltrim(sanitize_title($_REQUEST['cp_editor']), '/');
