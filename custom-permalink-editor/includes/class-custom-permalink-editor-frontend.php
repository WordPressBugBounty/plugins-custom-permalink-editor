...
class Custom_Permalink_Editor_Frontend
{
	...
	public function init()
	{
		...
		if (isset($_SERVER['REQUEST_URI'])) {
			...
			$this->request_uri = sanitize_url($_SERVER['REQUEST_URI']);
		}
	...
	private function query_post($requested_url)
	{
		...
		if (! $posts) {
			...
			// Add proper escaping for LIKE queries:
	...
	public function parse_request($query)
	{
		...
		if (
			...
			$this->request_uri = filter_var($_SERVER['REQUEST_URI'], FILTER_SANITIZE_URL);
			$this->request_uri = wp_unslash($this->request_uri);
		}
	...
	public function make_redirect()
	{
		...
		if (
			! isset($posts[0]->ID) || ! isset($posts[0]->meta_value)
			...
			if ((is_single() || is_page()) && ! empty($wp_query->post)) {
				...
				// For custom post types, only ignore if there's no existing custom URL
				if (isset($post->post_type) && 'post' !== $post->post_type && 'page' !== $post->post_type) {
					// Keep existing custom URLs for custom post types, but don't allow new ones
					if (empty($cp_editor)) {
						$cp_editor = '';
					}
				}
			...
			// For custom post types, only ignore if there's no existing custom URL
			if (isset($posts[0]->post_type) && 'post' !== $posts[0]->post_type && 'page' !== $posts[0]->post_type) {
				// Keep existing custom URLs for custom post types, but don't allow new ones
				if (empty($cp_editor)) {
					$cp_editor = '';
				}
		...
		if (
			...
			if (
				substr($request, 0, $original_length) === $original_permalink
	...
	public function custom_post_link($permalink, $post)
	{
		...
		// Allow custom permalink for posts, pages, AND custom post types with existing custom URLs
		if ($cp_editor && isset($post->post_type)) {
			// For custom post types, only apply if there's an existing custom URL
			if ('post' === $post->post_type || 'page' === $post->post_type || !empty($cp_editor)) {
				$post_type = 'post';
